...
PATTERNS = {
    ...
    "tax": re.compile(r"\b0\.90\b|tax|after_tax", re.IGNORECASE),
    "flip": re.compile(r"flip_economics|compute_flip|after_tax_sale|profit / bid", re.IGNORECASE),
    "upgrade": re.compile(r"roster_upgrade|score_upgrade|p_cross|distance_to", re.IGNORECASE),
}
...
def audit_repo(root: str | Path) -> dict[str, Any]:
    ...
    for path in sorted(root_path.rglob("*")):
        if not path.is_file() or path.suffix.lower() not in {".r", ".py"}:
            continue
...
PARITY_FEATURES = [
    {
        ...
        "group": "Market Scan",
        "shiny_source": "R/server_scan.R::scan_partition",
        ...
        "required_terms": ["flip_buys", "upgrade_buys", "holds", "sells", "dropped"],
    },
    {
        "feature_id": "market_scan.partition_columns",
        "group": "Market Scan",
        "shiny_source": "R/server_scan.R::render_table",
    ...
    },
    {
        ...
        "group": "Market Scan",
        "shiny_source": "R/server_scan.R::Progress",
        "react_source": "frontend/src/tabs/ScanTab.tsx",
    ...
    },
    {
        ...
        "group": "Market Scan",
        "shiny_source": "R/server_scan.R::scan_uuid_paste",
    ...
    },
    {
        ...
        "group": "Card Analysis",
        "shiny_source": "R/server_card.R::forecast plot",
    ...
    },
    {
        ...
        "group": "Card Analysis",
        "shiny_source": "R/server_card.R::walk_forward_cv",
        "react_source": "frontend/src/tabs/CardTab.tsx",
    ...
    },
    {
        ...
        "group": "Validate",
        "shiny_source": "R/server_validate.R::manual flip",
    ...
    },
    {
        ...
        "group": "Overall",
        "shiny_source": "R/server_overall.R::overall_market_health",
...
def parity_markdown(payload: dict[str, Any]) -> str:
    ...
    for row in payload.get("features", []):
        lines.append(
            ...
            f"`{row['shiny_source']}` | `{row['react_source']}` / `{row['python_source']}` |"
        )
    return "\n".join(lines) + "\n"
