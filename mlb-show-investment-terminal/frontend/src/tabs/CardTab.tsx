...
import { finalAction, InventoryRiskPanel, ProvenancePanel, StrategySummary } from "../strategyComponents";
    ...
    <div className="grid">
      <Panel title="Find Card" kicker={<span>The Show API proxy</span>}>
        <div className="form-row">
          <label>Player name<input value={name} onChange={(e) => setName(e.target.value)} aria-label="Player name" /></label>
        ...
        {search.error ? <div className="error" role="alert">{search.error.message}</div> : null}
      ...
      <Panel title="Analyze Listing" kicker="same-origin /api/card/analyze">
        <div className="form-row">
          <label>Card UUID<input value={uuid} onChange={(e) => setUuid(e.target.value)} placeholder="32-hex UUID" aria-label="Card UUID" /></label>
          <button onClick={() => load.mutate(uuid)} disabled={!uuid || load.isPending}>Load UUID</button>
        </div>
        <textarea value={rawJson} onChange={(e) => setRawJson(e.target.value)} placeholder="Paste listing.json here" rows={7} aria-label="Listing JSON" />
      ...
      {result ? <InventoryRiskPanel record={result} /> : null}
      {result ? <ProvenancePanel record={result} /> : null}
...
function TargetPanel({ record }: { record: ScoreRecord }) {
  ...
  const panelClass = status === "INVESTABLE"
    ? "target-panel target-investable"
    : status === "OBSERVATIONAL ONLY"
      ? "target-panel target-observational"
      : "target-panel target-blocked";
  const decision = finalAction(record);
  ...
  return (
    <Panel title="Target / Trade Ticket" kicker={<FreshnessBadge at={record.fetched_at ?? record.provenance?.pull_timestamp} label="LISTING" />} className={panelClass}>
      ...
      <div className="target-signal-row">
        ...
        <div>
          ...
          <SignalPill action={governedAction(record, record.upgrade?.action)} />
        </div>
      ...
      <div className="stat-grid">
        ...
        <Stat label="Profit" value={profit} tone={(record.flip?.profit ?? 0) > 0 ? "good" : "neutral"} />
...
function DiagnosticsPanel({ record }: { record: ScoreRecord }) {
  ...
  const failedGates = gateEntries
    .filter(([, g]) => g && !g.passed)
    .map(([k]) => k);
  const status = verdictOf(record);
  const gatesLabel = gateEntries.length
    ? failedGates.length ? `${failedGates.length} failed` : "all pass"
    : status === "INVESTABLE" ? "all pass" : "not cleared";
  const gatesTone = gatesLabel === "all pass" ? "good" : status === "NOT INVESTABLE" ? "bad" : "warn";
  return (
    <Panel title="Forecast Diagnostics" kicker={`verdict: ${status}`}>
      <div className="stat-grid">
        ...
        <Stat label="Forecast EV" value={blankIf(record, fmtPct(record.forecast?.expected_ret, 2))} />
        <Stat label="Forecast P(profit)" value={blankIf(record, fmtPct(record.forecast?.p_profit, 1))} />
        ...
        <Stat label="Gates" value={gatesLabel} tone={gatesTone} />
